...
- Fixed functions for plotting multiple confidence intervals (using logit
  instead of exp)
...
- When `se_log_hazard` is zero, the lower and upper CI for the hazard rate are
  fixed at zero as well. The reason for doing this is that having `NA`s for
  those quantities was causing unexpected behavior on the plots.
...
- `calc_cdf` function also esimates the density (in addition to the CDF) and its
  ...
  respective density and CDF evaluations.
  
- `summary` and `plot` methods for the output of the `calc_cdf` function.

- $\hat{f}$ is no longer part of the output

- $\hat{u}$ is now referred to as `risk_set`

- `ralife_cdf` function to sample "time to event" data has been included in the
  package.
